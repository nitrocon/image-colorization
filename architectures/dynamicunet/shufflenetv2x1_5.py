...
def build_shufflenet_unet(n_input=1, n_output=2, size=None):
    shufflenet = shufflenet_v2_x1_5(weights=ShuffleNet_V2_X1_5_Weights.DEFAULT)
    body = create_body(shufflenet, pretrained=True, n_in=n_input, cut=-2)
    net_G = DynamicUnet(body, n_output, (size, size))
    return net_G

class ShuffleNetV2x1_5DynamicUnet(torch.nn.Module):
    def __init__(self, device, size=None):
        super(ShuffleNetV2x1_5DynamicUnet, self).__init__()
        self.device = device
        
        with tqdm(total=2, desc="Initializing ShuffleNetV2 X1.5 Dynamic-UNet") as pbar:
        ...
        print("ShuffleNetV2 X1.5 Dynamic-UNet model initialized and moved to device.")
